...
export function TicketsWorkspace() {
  ...
  const filtered = useMemo(() => {
    ...
    return tickets.filter((ticket) =>
      [
        ...
        ticket.method,
        ticket.customer,
        ...ticket.items.map((item) => item.code),
      ]
        .join(" ")
        .toLowerCase()
        .includes(term),
  ...
  function findScannedTicket(code: string) {
    const saleFolio = saleFolioFromReceiptCode(code);
    setQuery(saleFolio);
    const ticket = tickets.find(
      (row) => row.id.toLocaleUpperCase("es-MX") === saleFolio,
    );
    if (ticket) selectTicket(ticket);
  }

  useKeyboardBarcodeScanner(findScannedTicket);

  return (
    <section className="module-page tickets-page">
      <div className="section-heading">
        <div>
          <p className="eyebrow">Ventas de hoy</p>
          <h1>Tickets y comprobantes</h1>
          <p className="heading-copy">
            Consulta ventas, reimprime el ticket o genera la copia sin precios
            para regalo.
          </p>
        </div>
      </div>
      <div className="toolbar-card">
        <label className="module-search">
          ...
          <input
            ...
            onKeyDown={(event) => {
              if (event.key === "Enter") findScannedTicket(query);
            }}
            placeholder="Buscar folio, método o código"
            aria-label="Buscar tickets"
          />
        </label>
        <label className="toolbar-select">
          <span>Periodo</span>
          <select aria-label="Filtrar periodo" defaultValue="today">
        ...
        <label className="toolbar-select">
          <span>Caja</span>
          <select aria-label="Filtrar caja" defaultValue="register-1">
            <option value="register-1">Caja 01</option>
          </select>
        </label>
      </div>
        ...
        <section className="content-card ticket-list-card">
          ...
          {filtered.map((ticket) => (
            ...
            >
              ...
              <strong>{money.format(ticket.total)}</strong>
          ...
          {filtered.length === 0 ? (
            <div className="empty-list">
              ...
              <span>Prueba con otro folio o código.</span>
            </div>
          ) : null}
        </section>
        ...
        <aside className={selected ? "ticket-detail open" : "ticket-detail"}>
          {selected ? (
            <>
              ...
              <div className="receipt-paper-stage compact-stage">
                <ThermalReceipt
                  mode={receiptMode}
                  folio={selected.id}
                  date={`27/08/2026 ${selected.time}`}
                  items={receiptLines}
                  subtotal={receiptSubtotal}
                  discount={Math.max(0, receiptSubtotal - selected.total)}
                  total={selected.total}
                  method={selected.method}
                  tendered={selected.total}
                  change={0}
              ...
              <p className="demo-caption">
                Revisa la vista de 80 mm antes de imprimir. El ticket de regalo
                oculta precios y forma de pago.
              </p>
            </>
          ) : (
            <div className="detail-placeholder">
              ...
              <span>Aquí verás su detalle y opciones de impresión.</span>
            </div>
          )}
        </aside>
      </div>
