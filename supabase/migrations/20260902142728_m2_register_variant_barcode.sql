...
-- Registra códigos que no pertenecen al generador interno ni al importador
-- de SICAR. La variante se bloquea para serializar el cambio de primario y
-- conservar siempre una sola etiqueta vigente sin invalidar las anteriores.
...
comment on function public.register_variant_barcode(uuid, text, text, text) is
  'Registra códigos MANUAL/SUPPLIER, conserva anteriores y exige products.update.';

revoke execute on function public.register_variant_barcode(uuid, text, text, text)
  from public, anon;
grant execute on function public.register_variant_barcode(uuid, text, text, text)
  to authenticated;
