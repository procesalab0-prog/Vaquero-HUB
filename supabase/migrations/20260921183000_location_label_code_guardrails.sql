...
-- La clave de etiqueta se imprime en papel y viaja pegada a la mercancía.
-- Con el disparador anterior, un UPDATE que dejara `label_code` en nulo no
-- fallaba: acuñaba una clave nueva en silencio, y la sucursal cambiaba de
-- identidad impresa sin que nadie lo pidiera. Ahora un nulo sobre una fila que
-- ya tiene clave conserva la que ya estaba; sólo un alta nueva recibe una
-- automática.
...
begin
  ...
  -- Una fila que ya tenía clave no la pierde ni la cambia por venir vacía.
  if tg_op = 'UPDATE'
     and nullif(btrim(coalesce(old.label_code, '')), '') is not null then
    new.label_code := old.label_code;
    return new;
  end if;
...
-- Es una función interna de disparador. No forma parte de la API y no debe
-- heredar el EXECUTE que PostgreSQL concede a PUBLIC por omisión.
revoke execute on function app.assign_location_label_code()
from public, anon, authenticated, service_role;

-- Una clave de etiqueta repetida salía como violación cruda del índice único,
-- y la pantalla sólo podía decir «no fue posible guardar la sucursal». Ahora
-- el error tiene nombre y la interfaz puede explicar qué pasó.
...
begin
  ...
  if nullif(btrim(coalesce(p_label_code, '')), '') is not null
     and upper(btrim(p_label_code)) !~ '^[A-Z0-9]{1,4}$' then
    raise exception 'INVALID_LABEL_CODE' using errcode = '22023';
  end if;
  ...
  begin
    if p_id is null then
      ...
      if exists (
        select 1 from public.locations where code = upper(btrim(p_code))
      ) then
        raise exception 'LOCATION_CODE_TAKEN' using errcode = '23505';
      end if;
      insert into public.locations(
        code, label_code, name, type, address, phone, legal_name, tax_id,
        is_active
      ) values (
  ...
  exception
    when unique_violation then
      ...
      if v_constraint = 'locations_label_code_unique' then
        raise exception 'LABEL_CODE_TAKEN' using errcode = '23505';
      end if;
      if v_constraint = 'locations_code_key' then
        raise exception 'LOCATION_CODE_TAKEN' using errcode = '23505';
      end if;
      raise;
  end;
