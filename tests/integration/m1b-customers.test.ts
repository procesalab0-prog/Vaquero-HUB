...
describe.sequential("M1B: clientes, identidad y RLS", () => {
  beforeAll(async () => {
    ...
    const { data: location, error: locationError } = await server.from("locations").insert({ code: `C${runCode}`, name: "Clientes prueba", type: "STORE" }).select("id").single();
    expect(locationError).toBeNull();
    state.locationId = location!.id;

    for (const definition of [{ key: "admin", role: "ADMIN" }, { key: "cashier", role: "CASHIER" }, { key: "warehouse", role: "WAREHOUSE" }] as const) {
      ...
      const { error: profileError } = await server.from("app_users").insert({ id, employee_code: `M1B${definition.key.toUpperCase()}${runCode}`, full_name: `M1B ${definition.key}`, email, role_id: roleIds[definition.role] });
      expect(profileError).toBeNull();
      const { error: assignmentError } = await server.from("user_locations").insert({ user_id: id, location_id: state.locationId });
  ...
  it("permite búsqueda única por teléfono, socio y nombre", async () => {
    for (const query of ["4567", state.memberNumber, "Cliente de Prueba"]) {
      const { data, error } = await state.cashier!.client.rpc("search_customers", { p_query: query, p_limit: 10 });
      expect(error).toBeNull();
  ...
  it("almacén y anon no pueden consultar ni crear clientes", async () => {
    const warehouseSearch = await state.warehouse!.client.rpc("search_customers", { p_query: "Cliente", p_limit: 10 });
    const warehouseRows = await state.warehouse!.client.from("customers").select("id");
  ...
  it("ni ADMIN puede borrar clientes y la auditoría no duplica sus datos personales", async () => {
    const deleteAttempt = await state.admin!.client.from("customers").delete().eq("id", state.customerId);
    expect(deleteAttempt.error).not.toBeNull();

    const { data: audit, error } = await state.admin!.client.from("audit_log").select("before_data, after_data, metadata").eq("entity_type", "customers").eq("entity_id", state.customerId).single();
    expect(error).toBeNull();
